...
settings = {
    ...
    'SrTiO_2': {'test': True, 'multiple': 2, 'group': 195, 'top': 1, 'grid': 8, 'use_ml': False},
    'SrTiO_3': {'test': True, 'multiple': 3, 'group': 221, 'top': 1, 'grid': 6, 'use_ml': False},
    'SrTiO_4': {'test': True, 'multiple': 3, 'group': 200, 'top': 1, 'grid': 6, 'use_ml': False},
    'SrTiO_5': {'test': True, 'multiple': 3, 'group': 195, 'top': 1, 'grid': 6, 'use_ml': False},
...
# file path
#/home/yanghn/Application/src/work
...
def benchmark():
    ...
    for lib in libraries:
        ...
        for test_name in tests:
            ...
            ions_count = {}
            if lib == 'SrTiO':
                ions_count = {'Sr': 1 * multiple ** 3, 'Ti': 1 * multiple ** 3, 'O': 3 * multiple ** 3}
            elif lib == 'Y2O3':
                ions_count = {'Y': 2 * multiple ** 3, 'O': 3 * multiple ** 3}
            elif lib == 'MgAl2O4':
                ions_count = {'Mg': 1 * multiple ** 3, 'Al': 2 * multiple ** 3, 'O': 4 * multiple ** 3}
            elif lib == 'Ca3Al2Si3O12':
                ions_count = {'Ca': 3 * multiple ** 3, 'Al': 2 * multiple ** 3, 'Si': 3 * multiple ** 3, 'O': 12 * multiple ** 3}
            else:
                continue
            ...
            allocator = Allocate(ions_count, grid, 3.9*multiple, phase)
