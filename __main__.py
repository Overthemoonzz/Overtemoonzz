...
settings = {
    ...
    'SrTiO_1': {'test': False, 'multiple': 1, 'group': 1, 'top': 1, 'grid': 4, 'use_ml': False},
    ...
    'SrTiO_1_ml': {'test': True, 'multiple': 1, 'group': 1, 'top': 1, 'grid': 4, 'use_ml': True},
    ...
    with open(os.path.join("..", "results", test_name, "energies.txt"), "w+") as f:
        for i in range(len(results)):
            if final[i] is not None:
                ...
                ase.io.write(os.path.join("..", "results", test_name, f'solution{count}_minimised.vasp'), final_atoms)
                count += 1
...
def get_cif_energies(filename, library, format='cif'):
    ...
    The 'library' parameter is ignored in this version.
    """
    filedir = root_dir / 'structures/'
    
    # Ensure the filename has the correct library name
    if filename.startswith('SrTiO3') and library == 'SrTiO':
        filename = filename.replace('SrTiO3', 'SrTiO')
    ...
    try:
        ...
        print(f"Warning: Could not find CIF file at {file_path}")
        print(f"Trying alternative path...")
        # Try with SrTiO3 name as fallback
        alt_filename = filename.replace('SrTiO', 'SrTiO3')
        alt_path = os.path.join(filedir, alt_filename)
        print(f"Trying: {alt_path}")
        cryst = ase.io.read(alt_path, format=format, parallel=False)
        struct = AseAtomsAdaptor.get_structure(cryst)
        pot = matgl.load_model("M3GNet-MP-2021.2.8-PES")
        relaxer = Relaxer(potential=pot)
        relax_results = relaxer.relax(struct, fmax=0.05)
        energy = relax_results["trajectory"].energies[-1]
        print("The energy of", alt_filename, "is equal to", energy, "eV")
        return energy


def benchmark():
    # Prepare a folder for results
    print('Preparing folder for results')
    ...
    df_full = pd.DataFrame(
        columns=['name', 'grid', 'group', 'ions_count', 'atom_count', 'E_place', 'E_relax', 'time', 'config'])
    ...
    for lib in libraries:
        ...
        for test_name in tests:
            ...
            energy = get_cif_energies(filename=ref_filename, library=lib)
            if multiple > 1:
    ...
    # Save to file
    with open(os.path.join(root_dir, "results", "summary.txt"), "w+") as f:
