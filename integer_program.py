...
class Allocate:
    ...
    def optimize_cube_symmetry_ase(self, group='1', PoolSolutions=1, TimeLimit=0, verbose=True):
        ...
        if self.phase.garnet:
			...
			# Modified ML potential part replacing the classical potential
	        for ion_pair in self.phase.buck:  # (We keep the same keys to indicate which pairs to consider)
		        if not ((ion_pair[0] in types) and (ion_pair[1] in types)):
			        continue
                ml_matrix = get_ML_matrix(ion_pair, self.grid, self.cell, self.phase)
		        if ion_pair[0] == ion_pair[1]:
			        j1 = types.index(ion_pair[0])
			        for i1 in range(N):
			            for i2 in range(i1, N):
				            energy.add(Vars[j1][o_pos[i1]] * Vars[j1][o_pos[i2]] * ml_matrix[i1, i2])
                else:
                    j1 = types.index(ion_pair[0])
                    j2 = types.index(ion_pair[1])
                    for i1 in range(N):
                        for i2 in range(i1 + 1, N):
                            energy.add(Vars[j1][o_pos[i1]] * Vars[j2][o_pos[i2]] * ml_matrix[i1, i2])
                            energy.add(Vars[j2][o_pos[i1]] * Vars[j1][o_pos[i2]] * ml_matrix[i1, i2])
                del buck
