...
from ipcsp.matrix_generator import Phase, get_Ewald, get_Buck, get_garnet, get_ML_potential
import ase
...
class Allocate:
    ...
    def __init__(self, ions_count, grid_size, cell_size, phase):
        """
        Initialize an Allocate object for structure optimization.
        
        Args:
            ions_count (dict): Dictionary mapping ion types to their counts
    ...
    def solution_to_Atoms(self, i, orbits):
        ...
        for v in self.model.getVars():
            if v.Xn == 1:
                ...
                # And its orbit
                for pos in orbits[int(o)]:  # Convert o to integer to match orbits keys
    ...
    def optimize_cube_symmetry_ase(self, group='1', PoolSolutions=1, TimeLimit=0, verbose=True):
        ...
        # Extract the orbits dictionary
        if isinstance(orbit_data, dict) and "orbits" in orbit_data:
            orbits = orbit_data["orbits"]
        else:
            orbits = orbit_data
            
        # Convert string keys to integers
        orbits = {int(k): v for k, v in orbits.items()}
        
        # Get sorted keys
        orb_key = sorted(orbits.keys())
        ...
        for i in range(N):
            ...
            if not found:
                # For positions not explicitly included, assign to orbit 0
                # This is a fallback in case of incomplete orbit data
                o_pos.append(0)
        ...
        for i in range(O):  # iterate over all orbitals
            tmp_var = []
            for j in range(T):
                Vars[j] += [m.addVar(vtype=gb.GRB.BINARY, name=str(types[j]) + '_' + str(orb_key[i]))]
                tmp_var += [Vars[j][-1]]
            if i == 0:
        ...
        if self.phase.garnet:
            ...
            if self.phase.use_ml_potential:
                # ML Potential Part
                print("Using ML potential for short-range interactions")
                for i, ion1 in enumerate(types):
                    for j, ion2 in enumerate(types[i:], i):
                        ion_pair = (min(ion1, ion2), max(ion1, ion2))
                        
                        # Get ML potential matrix for this ion pair
                        ml_pot = get_ML_potential(ion_pair, self.grid, self.cell, self.phase)
                        ...
                        if ion1 == ion2:
                            ...
                            for i1 in range(N):
                                for i2 in range(i1, N):
                                    energy.add(Vars[j1][o_pos[i1]] * Vars[j1][o_pos[i2]] * ml_pot[i1, i2])
                        else:
                            j1 = types.index(ion1)
                            j2 = types.index(ion2)
                            ...
                            for i1 in range(N):
                                for i2 in range(i1 + 1, N):
                                    energy.add(Vars[j1][o_pos[i1]] * Vars[j2][o_pos[i2]] * ml_pot[i1, i2])
                                    energy.add(Vars[j2][o_pos[i1]] * Vars[j1][o_pos[i2]] * ml_pot[i1, i2])
        ...
        if m.status == gb.GRB.OPTIMAL or m.status == gb.GRB.TIME_LIMIT or gb.GRB.INTERRUPTED:
            ...
            # Only attempt to print variables if there are solutions
            if m.SolCount > 0:
                for v in m.getVars():
                    try:
                        # Try different attribute names that Gurobi might use
                        if hasattr(v, 'Xn') and v.Xn == 1:
                            print(v.varName, end=' ')
                        elif hasattr(v, 'X') and v.X == 1:
                            print(v.varName, end=' ')
                        elif hasattr(v, 'x') and v.x == 1:
                            print(v.varName, end=' ')
                    except Exception as e:
                        print(f"\nWarning: Could not check variable value: {e}")
            print()
            
            # Print the optimal objective value if available
            try:
                if hasattr(m, 'ObjVal'):
                    print('Minimal energy via optimizer: %g' % m.ObjVal)
                elif hasattr(m, 'objVal'):
                    print('Minimal energy via optimizer: %g' % m.objVal)
                else:
                    print('No objective value available')
            except Exception as e:
                print(f"Warning: Could not access objective value: {e}")

            if PoolSolutions > 1:
                # Get objective value with proper attribute name
                obj_val = None
                try:
                    if hasattr(m, 'ObjVal'):
                        obj_val = m.ObjVal
                ...
                try:
                    ...
                    obj_val = None
                return res[0:1], runtime, obj_val
